# MenuTitle: Brand New Day (with log file - v2)
# -*- coding: utf-8 -*-
...
def change_font_name(familyName=FONT.familyName):
    date_format = re.compile(r'\d{4}-\d{2}-\d{2}$')

    if date_format.search(familyName):
        striped_date = re.sub(date_format, '', familyName).strip()
    else:
        striped_date = familyName
...
# NEW: create a OTFs folder, if not there yet
if not check_path(LOG_OTF):
    create_folder(LOG_OTF)

# NEW: generate otf's for comparision
# but first, we need to chnage the fontfamily name
change_font_name()
generate_otf(LOG_OTF)
